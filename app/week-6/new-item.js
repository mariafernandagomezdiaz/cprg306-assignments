...
export default function NewItem({ onAddItem }) { 
  ...
  const handleSubmit = (event) => {
    ...
    onAddItem(newItem); // Llama a la función onAddItem
    ...
    <form onSubmit={handleSubmit} className="p-4 bg-blue-300 text-black rounded inline-block">
      <div className="flex flex-col space-y-4">
        ...
        <div className="flex space-x-4">
          <input type="number" value={quantity} min="1" max="99" onChange={handleQuantity} style={{width:'20%'}} required
            className="px-3 py-2 rounded"
          />
          <select onChange={handleCategory} value={category} style={{width:'30%'}} required
            className="px-3 py-2 rounded"
          >
